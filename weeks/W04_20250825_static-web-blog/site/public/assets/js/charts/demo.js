...
(() => {
  ...
  // src/ts/charts/demo.ts
  var mount = document.getElementById("chart");
  if (!mount) {
    console.warn("No #chart");
  } else {
    const data = [3, 1, 4, 1, 5, 9];
    const w = 640, h = 280, m = { top: 10, right: 10, bottom: 20, left: 30 };
    const x = band().domain(data.map((_, i) => String(i))).range([m.left, w - m.right]).padding(0.2);
    const y = linear2().domain([0, max(data)]).nice().range([h - m.bottom, m.top]);
    const svg = select_default2(mount).append("svg").attr("width", w).attr("height", h);
    svg.append("g").selectAll("rect").data(data).enter().append("rect").attr("x", (_, i) => x(String(i))).attr("y", (d) => y(d)).attr("width", x.bandwidth()).attr("height", (d) => y(0) - y(d)).attr("rx", 6).attr("class", "fill-neutral-300");
    svg.append("g").attr("transform", `translate(0,${h - m.bottom})`).call(axisBottom(x).tickSizeOuter(0)).attr("class", "text-neutral-400");
    svg.append("g").attr("transform", `translate(${m.left},0)`).call(axisLeft(y).ticks(5)).attr("class", "text-neutral-400");
  }
