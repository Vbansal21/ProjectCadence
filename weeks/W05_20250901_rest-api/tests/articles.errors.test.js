...
test("404 on get non-existent article", async () => {
  const res = await request(app).get("/articles/00000000-0000-4000-8000-000000000000");
  expect(res.status).toBe(404);
});

test("listArticles supports authorId filter + pagination", async () => {
  const u = await request(app).post("/users").send({ name: "Al", email: "a@a.com" });
  ...
  const list = await request(app).get(`/articles?authorId=${id}&page=1&limit=2&sort=createdAt:asc`);
